...
// Homed position variables
float homedPosition1 = 180.0;  // Motor 1 homed position (can be adjusted)
float homedPosition2 = 0.0;    // Motor 2 homed position (can be adjusted)

void IRAM_ATTR limitSwitch1Pressed() {
  motor1Homed = true;  // Set homed flag
  stepper1.stop();     // Stop motor 1
  stepper1.setCurrentPosition(homedPosition1);  // Set current position to the homed position
}

void IRAM_ATTR limitSwitch2Pressed() {
  motor2Homed = true;  // Set homed flag
  stepper2.stop();     // Stop motor 2
  stepper2.setCurrentPosition(homedPosition2);  // Set current position to the homed position
}
